...
class PathfindTile:
    ...
    def __init__(self, location: tuple[int], path: list[tuple], cost: int, dim: tuple[int]):
        ...
        # path to the tile (is also the fastest)
        self.path = path
        # cost of the path
        self.cost = cost
        # dimensions of the maze to not go out of bounds in self.adjacent(..)
        self.dim = dim

    def adjacent(self):
        # generates tiles that are adjacent to the current tile
...
class PathfindGrid:
    ...
    def pathfind(self, begin: tuple, end: tuple):
        edges: set[PathfindTile] = set([PathfindTile(begin, [begin], 0, self.shape)])
        visited: set[tuple[int]] = set()

        # indicates the path lenght
        min_path = 0

        while True:
            #print(min_path,visited)
            nxt_edges: set[PathfindTile] = set()
            for tile in edges:
                ...
                for adj, dir in tile.adjacent():
                    ...
                    if adj in visited:
                        continue
                    nxt_tile = PathfindTile(adj, tile.path + [adj], tile.cost + self.cost[dir], self.shape)
                    if adj == end:
...
if __name__ == "__main__":
    print(visualizePath("dwarfen-fortress/files/zauberschule5.txt"))